...
async function findOverlappingRecord(record, companyId, excludeId) {
  const params = [companyId, record.type, record.meterName.trim().toLowerCase(), record.periodStart, record.periodEnd];
  const excludeClause = excludeId ? `AND id <> $6` : "";
  if (excludeId) params.push(excludeId);

  const result = await query(
    `SELECT id, period_start, period_end, meter_name FROM utility_records WHERE facility_id = $1 AND type = $2 AND lower(trim(meter_name)) = $3 AND period_start <= $5::date AND period_end >= $4::date ${excludeClause} ORDER BY period_start DESC LIMIT 1`,
    params,
  );
