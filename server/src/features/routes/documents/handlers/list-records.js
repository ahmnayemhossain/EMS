...
export async function listDocumentRecords(req, res, next) {
  try {
    ...
    let where = "WHERE EXISTS (SELECT 1 FROM user_companies uc WHERE uc.user_id = $1 AND uc.company_id = dr.facility_id)";
    if (companyId) {
      params.push(companyId);
      where += ` AND dr.facility_id::text = $${params.length}`;
    }

    const result = await query(`${selectDocumentsSql} ${where} ORDER BY dr.created_at DESC`, params);
    res.json(result.rows.map(rowToDocument));
  } catch (error) {
    next(error);
  }
}
