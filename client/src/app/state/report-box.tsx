...
const RECORDS_KEY = "ems:reportBoxRecords_v1";
...
function safeParseRecords(raw: string | null) {
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw) as unknown;
    if (!Array.isArray(parsed)) return null;
    return parsed as ReportBoxRecord[];
  } catch {
    return null;
  }
}
...
export function ReportBoxProvider({ children }: { children: React.ReactNode }) {
  ...
  const [records, setRecords] = React.useState<ReportBoxRecord[]>(() => {
    const saved =
      typeof window !== "undefined"
        ? safeParseRecords(window.localStorage.getItem(RECORDS_KEY))
        : null;
    return saved && saved.length ? saved : [];
  });
  ...
  const setSubject = React.useCallback((id: string, subject: string) => {
    setReports((prev) =>
      prev.map((r) => (r.id === id ? { ...r, subject: subject } : r)),
    );
  }, []);
  ...
  const addRecord = React.useCallback(
    (reportId: string) => {
      ...
      const record: ReportBoxRecord = {
        ...
        reportId,
        recordedAt,
        title: report.subject?.trim() || "Complaint record",
        snapshot: report,
      };
  ...
  const removeRecord = React.useCallback((recordId: string) => {
  ...
  const setStatus = React.useCallback(
    (id: string, status: ReportBoxReport["status"], meta?: { handledBy?: string }) => {
      setReports((prev) =>
        prev.map((r) => {
          if (r.id !== id) return r;
          return {
            ...
            handledBy: status === "handled" ? meta?.handledBy || r.handledBy : undefined,
          };
        }),
      );
    },
