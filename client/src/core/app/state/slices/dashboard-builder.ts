...
export const useDashboardBuilder = create<DashboardBuilderStore>()(persist(createDashboardBuilderStore, { name: "ems-dashboard-builder-v2", version: 2, migrate: (persisted) => { const state = (persisted ?? {}) as PersistedDashboardBuilderState; return { containers: (state.containers ?? DEFAULT_DASHBOARD_BUILDER_STATE.containers).map((item, index) => ({ ...item, layout: clampRect(item.layout ?? { x: 1, y: 1 + index * 9, w: 12, h: 8 }) })), widgetsById: Object.fromEntries(Object.entries(state.widgetsById ?? DEFAULT_DASHBOARD_BUILDER_STATE.widgetsById).map(([id, widget]) => [id, { ...widget, rows: clampRows((widget as DashboardWidget).rows ?? 3) }])) }; } }));
