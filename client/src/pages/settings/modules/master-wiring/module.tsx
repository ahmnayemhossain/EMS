...
import { createSettingsEntity, deleteSettingsEntity, updateSettingsEntity } from "@/pages/settings/modules/settingsEntityApi";
import { buildEntityColumns, buildWiringColumns } from "@/pages/settings/modules/master-wiring/columns";
import { EntityFilterBar, WiringFilterBar } from "@/pages/settings/modules/master-wiring/filters";
import { MasterEntityForm, WiringForm } from "@/pages/settings/modules/master-wiring/forms";
import type { MasterWiringApi, MasterWiringConfig, WiringDraft } from "@/pages/settings/modules/master-wiring/types";
import { useMasterWiring } from "@/pages/settings/modules/master-wiring/use-master-wiring";
...
export function MasterWiringModule(props: { config: MasterWiringConfig; api: MasterWiringApi }) {
  ...
  return <div className="space-y-4"><EntityFilterBar search={vm.search} onSearchChange={vm.setSearch} refreshAction={<Button variant="outline" size="icon" onClick={() => void vm.loadAll()} disabled={vm.loading}><RefreshCw className="size-4" /></Button>} onClear={() => vm.setSearch("")} /><MasterSection config={props.config} vm={vm} rows={entityRows} columns={entityColumns} /><WiringSection config={props.config} vm={vm} rows={wiringRows} columns={wiringColumns} /><EditDialogs config={props.config} api={props.api} vm={vm} /><DeleteDialogs config={props.config} api={props.api} vm={vm} /></div>;
}

function MasterSection(props: any) {
...
function WiringSection(props: any) {
...
async function saveEntity(vm: any, kind: any, mode: "create" | "update") {
  ...
  const duplicate = vm.entityRows.some((row: any) => row.id !== draft.id && row.name.trim().toLowerCase() === draft.name.trim().toLowerCase());
...
async function saveWiring(vm: any, config: MasterWiringConfig, api: MasterWiringApi, mode: "create" | "update") {
  ...
  const duplicate = vm.wiringRows.some((row: any) => row.id !== draft.id && row.utilityTypeId === draft.utilityTypeId && row.relationId === draft.relationId);
