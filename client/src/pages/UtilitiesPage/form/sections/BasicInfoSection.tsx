...
import { utilityTypeFieldConfig, utilityTypes } from "@/pages/UtilitiesPage/constants";
import { DatePickerField } from "@/pages/UtilitiesPage/form/date-picker-field";
import { FieldError, FieldLabel, FormSection } from "@/pages/UtilitiesPage/form/form-ui";
import type { UtilityFormErrors, UtilityFormProps } from "@/pages/UtilitiesPage/form/types";
  ...
  return (
    <FormSection title="Basic Info">
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <div className="grid gap-1.5"><FieldLabel required={sourceEnabled}>Source</FieldLabel><SelectFilter value={props.sourceId} onChange={props.onSourceChange} placeholder={props.sourceOptions.length ? "Select source" : "No source configured"} items={props.sourceOptions.map((item) => ({ value: item.id, label: item.name }))} disabled={!sourceEnabled} /><FieldError>{errors.source}</FieldError></div>
        <div className="grid gap-1.5"><FieldLabel required>UOM</FieldLabel><SelectFilter value={props.unit} onChange={props.onUnitChange} placeholder={props.uomOptions.length ? "Select UOM" : "No UOM configured"} items={props.uomOptions.map((item) => ({ value: item.name, label: item.name }))} /><FieldError>{errors.unit}</FieldError></div>
        ...
        <div className="grid gap-1.5 sm:col-span-2"><FieldLabel required>{config.meterLabel}</FieldLabel><Input value={props.meterName} onChange={(event) => props.onMeterNameChange(event.target.value)} placeholder="e.g. Main incomer" /><FieldError>{errors.meterName}</FieldError></div>
      </div>
    </FormSection>
  );
}
