import { BellRing, ClipboardList, Droplet, FileWarning, ShieldCheck, TrendingUp } from "lucide-react";
...
import { audits, capas, chemicals, documents, facilities, notifications, utilityRecords } from "@/core/data/catalog/mock";
...
export function buildDashboardData() {
  ...
  const overdueActions: TimelineItem[] = capas.filter((item) => item.status === "overdue").map((item) => ({ id: item.id, title: item.title, date: `Due ${formatDate(item.dueDate)}`, description: `${item.owner} • Evidence: ${item.evidenceCount}`, tone: "critical" }));
