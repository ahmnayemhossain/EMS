...
export function buildDashboardData() {
  ...
  const overdueActions: TimelineItem[] = capas.filter((item) => item.status === "overdue").map((item) => ({ id: item.id, title: item.title, date: `Due ${formatDate(item.dueDate)}`, description: `${item.owner} â€¢ Evidence: ${item.evidenceCount}`, tone: "critical" }));
