...
export function buildDashboardData() {
  const avgReadiness = facilities.length
    ? Math.round(
        facilities.reduce(
          (sum, facility) => sum + facility.auditReadinessScore,
          0,
        ) / facilities.length,
      )
    : 0;
  ...
  const overdueActions: TimelineItem[] = capas
    ...
    .map((item) => ({
      ...
      description: `${item.owner} • Evidence: ${item.evidenceCount}`,
      tone: "critical",
    }));
  ...
  return {
    ...
    overdueActions,
    recentUploads,
    expiringDocuments: documents.filter((item) => item.status !== "valid"),
    facilities,
  };
}
