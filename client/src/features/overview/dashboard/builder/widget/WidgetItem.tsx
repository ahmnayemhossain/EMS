...
export function WidgetItem(props: { widgetId: string; containerId: string; index: number; enabled: boolean; gridRef: React.RefObject<HTMLElement | null>; span: number; rows: number; minSpan?: number; maxSpan?: number; minRows?: number; maxRows?: number; title?: string; onMove: (widgetId: string, from: { containerId: string; index: number }, to: { containerId: string; index: number }) => void; onSpanChange: (span: number) => void; onRowsChange: (rows: number) => void; onRemove?: () => void; children: React.ReactNode; }) {
  ...
  const body = <div ref={bindItemRef} className={cn("group relative min-w-0", isDragging ? "opacity-70" : undefined)} style={{ gridColumn: `span ${clampedSpan} / span ${clampedSpan}`, gridRow: `span ${clampedRows} / span ${clampedRows}` }}><WidgetShell enabled={props.enabled} title={props.title} dragHandleRef={dragRef}>{props.children}</WidgetShell>{props.enabled ? <><div onPointerDown={startResize("x")} className={cn("absolute right-0 top-1/2 z-10 flex h-14 w-6 -translate-y-1/2 items-center justify-center", "cursor-ew-resize touch-none text-muted-foreground/75 hover:text-foreground", "opacity-0 group-hover:opacity-100")} title="Resize width" role="separator" aria-orientation="horizontal"><ResizeHandleIcon className="size-3.5" /></div><div onPointerDown={startResize("y")} className={cn("absolute bottom-0 left-1/2 z-10 flex h-6 w-14 -translate-x-1/2 items-center justify-center", "cursor-ns-resize touch-none text-muted-foreground/75 hover:text-foreground", "opacity-0 group-hover:opacity-100")} title="Resize height" role="separator" aria-orientation="vertical"><ResizeHandleIcon className="size-3.5 rotate-90" /></div><div onPointerDown={startResize("xy")} className={cn("absolute bottom-0 right-0 z-10 flex h-8 w-8 items-center justify-center", "cursor-nwse-resize touch-none text-muted-foreground/75 hover:text-foreground", "opacity-0 group-hover:opacity-100")} title="Resize"><ResizeHandleIcon className="size-3.5" /></div></> : null}</div>;
