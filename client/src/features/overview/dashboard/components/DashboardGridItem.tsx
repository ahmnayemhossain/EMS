...
export function DashboardGridItem<TId extends string>({
  ...
  const { isResizing, startResize } = useDashboardGridResize({ enabled, gridRef, span, minSpan, maxSpan, onSpanChange });
  const { isDragging, dragRef, previewRef, dropRef } = useDashboardGridDnd({
    dndType, id, index, enabled, isResizing, itemRef, onMove,
  });
      ...
      {enabled ? (
        <>
          <div
            ...
            tabIndex={0}
            aria-label="Drag"
            className={cn("absolute left-2 top-2 z-10 inline-flex size-8 items-center justify-center rounded-md border bg-background shadow-sm", "cursor-grab active:cursor-grabbing opacity-100 transition-opacity touch-none")}
          >
            <GripVertical className="size-4 text-muted-foreground" />
          </div>
          <div
            onPointerDown={startResize}
            className={cn("absolute bottom-2 right-2 z-10 size-8 rounded-md border bg-background shadow-sm", "cursor-ew-resize opacity-100 transition-opacity touch-none")}
            title="Resize"
            role="separator"
