...
export function AuditCalendarPage() {
  ...
  return (
    <div className="flex min-h-[calc(100svh-8.5rem)] flex-col gap-4">
      ...
      <DetailPanel open={Boolean(vm.selectedDateKey && vm.auditsForSelectedKey.length)} onOpenChange={(open) => !open && vm.setSelectedDateKey(null)} title={vm.selectedDateKey ? `Audits ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ ${formatDate(vm.selectedDateKey)}` : "Audits"} description={selectedDescription}>{vm.selectedDateKey && vm.auditsForSelectedKey.length ? <AuditDetailContent audits={vm.auditsForSelectedKey} /> : null}</DetailPanel>
    </div>
  );
}
