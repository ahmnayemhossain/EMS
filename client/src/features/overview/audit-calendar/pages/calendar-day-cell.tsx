...
export function CalendarDayCell(props: {
  ...
  return (
    <button
      type="button"
      onClick={() => props.onSelectDay(props.dayKey, props.count > 0)}
      className={[
        props.cellClass,
        !monthMatch ? "opacity-55" : "",
      ...
      <div className="flex h-full flex-col items-start justify-between gap-2">
        <div className="flex w-full items-start justify-between">
          <div
            className={[
              props.dayPillClass,
              isToday ? "bg-[var(--calendar-accent)] text-white" : "text-foreground/90",
            ].join(" ")}
          >
            {props.day.getDate()}
          </div>
        </div>
        {props.count > 0 && primary ? (
          <AuditBadge {...props} primary={primary} />
        ) : (
  ...
  return (
    <div className="w-full space-y-1">
      <div
        ...
        title={props.primary.name}
      >
        <span className={["h-4 w-1 rounded-full", dotClass].join(" ")} aria-hidden />
        <span className="min-w-0 flex-1 truncate">
          {props.primary.time?.start
            ? `${props.primary.time.start}${props.primary.time.end ? `–${props.primary.time.end}` : ""} `
            : ""}
          {props.primary.name}
        </span>
      ...
      {props.label ? (
        <div className="text-muted-foreground text-[11px]">
          {props.label}
          {props.dayAudits.length > 1 ? ` • +${props.dayAudits.length - 1} more` : ""}
        </div>
      ) : null}
    </div>
  );
