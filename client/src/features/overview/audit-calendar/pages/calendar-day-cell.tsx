...
function AuditBadge(props: any) {
  ...
  return <div className="w-full space-y-1"><div className={["flex w-full items-center gap-2 overflow-hidden rounded-md border px-2 py-1 text-[11px] font-medium", badgeClass].join(" ")} title={props.primary.name}><span className={["h-4 w-1 rounded-full", dotClass].join(" ")} aria-hidden /><span className="min-w-0 flex-1 truncate">{props.primary.time?.start ? `${props.primary.time.start}${props.primary.time.end ? `ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ${props.primary.time.end}` : ""} ` : ""}{props.primary.name}</span><span className="text-muted-foreground text-xs tabular-nums">{props.count}</span></div>{props.label ? <div className="text-muted-foreground text-[11px]">{props.label}{props.dayAudits.length > 1 ? ` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¢ +${props.dayAudits.length - 1} more` : ""}</div> : null}</div>;
}
