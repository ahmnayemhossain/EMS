...
export function useWastePage() {
  ...
  const disposalTimeline: TimelineItem[] = rows.filter((item) => item.disposalStatus !== "disposed").map((item) => ({ id: item.id, title: `${item.stream} â€¢ ${formatNumber(item.qtyKg)} kg`, date: item.dueBy ? `Due ${formatDate(item.dueBy)}` : `Logged ${formatDate(item.date)}`, description: `${getFacilityName(item.facilityId)} â€¢ ${item.vendor ?? "Vendor not assigned"}`, tone: item.type === "hazardous" ? "warning" : "info" }));
