          ...
          rows.map((r) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => onSelect(r)}
                className={cn("w-full text-left px-3 py-3 hover:bg-muted/30 active:bg-muted/40")}
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <div className="truncate text-sm font-medium">{getCompanyName(r.facilityId)}</div>
                      <StatusBadge tone={tone} className="shrink-0">
                        {r.varianceFlag ?? "normal"}
                      </StatusBadge>
                    </div>
                    <div className="text-muted-foreground mt-1 truncate text-xs">{r.meterName}</div>
                    <div className="text-muted-foreground mt-2 text-[11px]">
                      {formatDate(r.periodStart)} – {formatDate(r.periodEnd)}
                    </div>
                    {r.sourceName ? (
                      <div className="text-muted-foreground mt-1 text-[11px]">Source: {r.sourceName}</div>
                    ) : null}
                  </div>
