...
import { formatFindingTimeline } from "./finding-timeline";

export function AuditFindingsCard({ audit }: { audit: AuditRecord }) {
  return (
    <div className="rounded-lg border p-3">
      ...
      <div className="mt-3 space-y-2">
        {audit.findings.slice(0, 8).map((finding) => {
          const person = finding.responsiblePerson ?? finding.owner;
          const timeline = formatFindingTimeline(finding);
          return (
            <div key={finding.id} className="rounded-md border p-2">
