...
import { facilities, getFacilityName, wasteRecords } from "@/core/data/mock";
import { formatDate, formatNumber } from "@/core/utils/format";
import type { TimelineItem } from "@/core/components/TimelineList";
...
export function useWastePage() {
  ...
  const disposalTimeline: TimelineItem[] = rows.filter((item) => item.disposalStatus !== "disposed").map((item) => ({ id: item.id, title: `${item.stream} • ${formatNumber(item.qtyKg)} kg`, date: item.dueBy ? `Due ${formatDate(item.dueBy)}` : `Logged ${formatDate(item.date)}`, description: `${getFacilityName(item.facilityId)} • ${item.vendor ?? "Vendor not assigned"}`, tone: item.type === "hazardous" ? "warning" : "info" }));
