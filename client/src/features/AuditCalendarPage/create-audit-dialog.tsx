...
import { auditTemplates, auditors, durationOptions } from "@/features/AuditCalendarPage/constants";
...
function DurationField(props: { duration: string; computedEndTime?: string; conflict: boolean; onChange: (value: string) => void }) {
  return <Field label="Duration"><SelectFilter value={props.duration} onChange={props.onChange} placeholder="Duration" items={durationOptions.map((value) => ({ value, label: `${value} min` }))} /><div className="text-muted-foreground text-xs tabular-nums">Ends at {props.computedEndTime ?? "—"}</div>{props.conflict ? <div className="text-destructive text-xs font-medium">Time overlaps another audit for this day.</div> : null}</Field>;
}
