...
import { AuditCalendarView } from "@/features/AuditCalendarPage/calendar-view";
import { CreateAuditDialog } from "@/features/AuditCalendarPage/create-audit-dialog";
import { AuditDetailContent } from "@/features/AuditCalendarPage/detail-panel-content";
import { toPickerDate } from "@/features/AuditCalendarPage/helpers";
import { AuditCalendarToolbar } from "@/features/AuditCalendarPage/toolbar";
import { UpcomingListView } from "@/features/AuditCalendarPage/upcoming-list-view";
import { useAuditCalendarPage } from "@/features/AuditCalendarPage/use-audit-calendar-page";
...
export function AuditCalendarPage() {
  ...
  return (
    <div className="flex min-h-[calc(100svh-8.5rem)] flex-col gap-4">
      ...
      <DetailPanel open={Boolean(vm.selectedDateKey && vm.auditsForSelectedKey.length)} onOpenChange={(open) => !open && vm.setSelectedDateKey(null)} title={vm.selectedDateKey ? `Audits • ${formatDate(vm.selectedDateKey)}` : "Audits"} description={selectedDescription}>{vm.selectedDateKey && vm.auditsForSelectedKey.length ? <AuditDetailContent audits={vm.auditsForSelectedKey} /> : null}</DetailPanel>
    </div>
  );
}
