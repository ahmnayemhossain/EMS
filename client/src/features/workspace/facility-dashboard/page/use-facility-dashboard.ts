...
export function useFacilityDashboard() {
  ...
  return { facility, facilityWastewater, facilityDocs, facilityIncidents, water: `${formatNumber(facilityUtilities.find((item) => item.type === "water")?.value ?? 0)} mÂ³`, electricity: `${formatNumber(facilityUtilities.find((item) => item.type === "electricity")?.value ?? 0)} kWh`, hazardousChem: facilityChemicals.filter((item) => item.approvalStatus !== "approved").length, wasteKg: `${formatNumber(facilityWaste.reduce((sum, item) => sum + item.qtyKg, 0))} kg`, openNonCompliance: facilityCapas.filter((item) => item.status !== "closed").length, dueTests: facilityWastewater.some((item) => item.exceedance?.length) ? 1 : 0 };
}
