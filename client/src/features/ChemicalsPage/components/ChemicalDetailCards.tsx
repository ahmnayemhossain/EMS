  ...
  return (
    <>
      ...
      <div className="rounded-lg border p-3">
          ...
          {chemical.sdsId ? <Button asChild variant="outline" size="sm"><Link to={`/sds?sdsId=${encodeURIComponent(String(chemical.sdsId))}`}>Open SDS<ExternalLink className="ml-2 size-4" /></Link></Button> : <StatusBadge tone="warning"><AlertTriangle className="size-3" />Missing SDS</StatusBadge>}
        </div>
      </div>
    </>
  );
