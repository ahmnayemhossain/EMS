...
export function usePublicReportBoxMessages(args: any) {
  ...
  const saveEdit = React.useCallback(async () => {
    ...
    const ok = await editInboxMessage({
      reportId: args.state.sessionReportId,
      messageId: args.state.editTarget.serverMessageId,
      text: args.state.composer.trim(),
    });
    args.state.setSending(false);

    if (ok) {
      updateLocalMessage(args.state.editTarget.localId, { text: args.state.composer.trim() });
      args.state.setComposer("");
      args.state.setEditTarget(null);
    }
  ...
  const sendText = React.useCallback(async () => {
    ...
    const res = await submitToInbox({
      ...
      reportId: args.state.sessionReportId || undefined,
      kind: "text",
      text,
      at,
    });
  ...
  const onPickPhoto = React.useCallback(async (file: File | null) => {
    ...
    const workerId = pushLocalMessage({
      at,
      kind: "photo",
      attachment: { mime: file.type || "image/*", name: file.name, dataUrl },
      from: "worker",
      status: "sending",
    });
    args.state.setSending(true);
...
async function handleSendResult({ res, workerId, pushLocalMessage, updateLocalMessage, state, refs }: any) {
  ...
  pushLocalMessage({ at: new Date().toISOString(), kind: "text", text: BN.sendFailed, from: "ems", status: "error" });
}
